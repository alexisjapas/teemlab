//! `train` — generate the trained-MLP showcase (`07_learning`) from a training scenario.
//!
...
//!   - `scenarios/examples/07_learning.ron` — a **self-contained** showcase: the trained
...
fn main() {
    ...
    let scenario = args
        .get(1)
        .cloned()
        .unwrap_or_else(|| "scenarios/examples/10_breeding.ron".into());
    ...
    // (2) Self-contained showcase `07_learning.ron`: the TRAINED MLP (sp0, a frozen
...
// GENERATED by `cargo run --bin train` — do not hand-edit; re-run to regenerate.\n";
    std::fs::write(
        "scenarios/examples/07_learning.ron",
        format!(
            "{evolved_header}{}",
            evolved.to_ron_string().expect("serialize evolved scenario")
        ),
    )
    .expect("write 07_learning.ron");

    println!(
        "wrote species/examples/mlp_trained.ron + scenarios/examples/07_learning.ron \
