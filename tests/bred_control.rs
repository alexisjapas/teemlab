...
const NAIVE: &str = include_str!("../scenarios/examples/06_learning.ron");
const TRAINED: &str = include_str!("../scenarios/examples/06_learning.ron");
/// The bred archetype (`species/saved/mlp_bred.ron`) spliced over archetype 0 of
/// `06_learning.ron` (count set back to 14); gitignored local state, hence read at
...
fn bred_vs_naive_against_the_wander_control() {
    let bred = std::fs::read_to_string(BRED_PATH).expect(
        "scenarios/saved/mlp_bred_control.ron missing — rebuild it by splicing the \
             species/saved/mlp_bred.ron archetype over archetype 0 of 06_learning.ron \
