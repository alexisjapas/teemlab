...
intraspécifique — la primitive d'interaction réutilisée, sans mécanisme nouveau. Et depuis la
**Phase 3b, le type spécial `Food` est dissous** : il ne reste qu'`Archetype` (un agent), une
*source de nourriture* étant un patch **sessile photosynthétique** sans reproduction —
renouvelable sur place, plus de robinet `replenish_food`. Au passage, la primitive
d'interaction **conserve l'énergie sous contention** (N fourrageurs sur un même patch se
partagent sa réserve, au lieu de la dupliquer).

**Reste.** **P5 — bataille** (régime générationnel, test final de l'abstraction le long d'une
*couture A/B* propre).
...
src/
  ...
  snapshot.rs     Snapshot d'une run (état vivant sérialisable) : config + RNG + agents (sources sessiles comprises).
  movement.rs     Systèmes percevoir / décider / agir (FixedUpdate, chaînés).
  interaction.rs  Primitive d'interaction unique (prédation / combat / compétition), conservée sous contention, + table de relations.
