...
scenarios/        Three categories; only examples/ and deferred/ are committed (saved/ is local).
  examples/       Curated, committed TEACHING set — a concise, PROGRESSIVE non-MLP arc
    # (simplest → most complex; the Open ▸ Examples menu sorts by name), map size scaled to
    # each. Its device is the SPLIT ARENA: an impassable rock wall halves the arena so a
    # contrast shows in ONE frame instead of across runs (02, 03, 05 are splits).
    01_meadow.ron       Producers on a scarce nutrient: sessile photosynthesisers whose photosynthesis CONSUMES a diffusing nutrient (Liebig, `metabolic_cost`) → a real carrying capacity with turnover, not a carpet. Cf. tests/flora.rs, tests/nutrients.rs.
    02_divide.ron       SPLIT (diagonal wall) — trophic dependence in one frame: the same grazer runs in both triangular halves, but only one has producers; that herd persists, the bare-side herd starves. Cf. tests/divide.rs.
    03_selection.ron    SPLIT (diagonal wall) — a priced, UNUSED trait decays only where it can: identical far-sighted WANDERERS on both halves; the mutable-vision half has its (costed) eyes melted down by selection while the frozen control holds. Cf. tests/selection.rs.
    04_hunt.ron         Vision pays only when USED: a Hunter and a Wander with the SAME eyes and cost share one flora — the Hunter steers on its vision and out-competes the Wander that can't. The 03→04 pair. Cf. tests/obstacle.rs.
    05_restraint.ron    SPLIT (diagonal wall) — restraint as a stability lever: a prudent grazer monoculture persists while an identical greedy one boom-busts to extinction, side by side (mix them and greed out-competes — the commons). Cf. tests/restraint.rs.
  deferred/       Committed but OUT of the teaching menu — parked pending later work (ROADMAP §0), still test fixtures / `train` output:
    learning.ron        (MLP, deferred) An evolved MLP forager (frozen `captured_brain`) vs a WANDER control on shared oases — neuroevolution reaching viable parity. GENERATED by `train`. Cf. tests/mlp.rs, tests/cohabitation.rs.
    foodweb.ron         (3-level web, deferred: returns with multi-nutrient metabolism) Three emergent trophic levels from one rule: flora → herbivore → carnivore. Cf. tests/predator_prey.rs.
    reef.ron            (deferred) Size-selective rock refugia + anchored kelp + uprooting/grazing turnover (detritus). Cf. tests/reef.rs.
    breeding.ron        (MLP, deferred) Run → score → breed: a cohort of headless matches per generation, scored by standing biomass; the best survivors re-seed the next cohort. Carries a `batch` — run with the `breed` bin.
...
play scenarios/examples/03_selection.ron          # debug, explicit scenario
play --release                                 # release (teemlab AND record in release)
play --release scenarios/examples/01_meadow.ron   # profile + explicit scenario

cargo run --bin headless                                   # headless, default scenario
cargo run --bin headless scenarios/examples/02_divide.ron     # explicit scenario (1st arg = RON)

# Record a run to video (headless render → ffmpeg); output in outputs/:
cargo run --bin record -- scenarios/examples/04_hunt.ron --out outputs/run.mp4
#   options: --out F  --fps N  --seconds S  --width W  --height H  --components
#   (defaults: 30 fps, 61 s, 1080×1080 — the arena is square)
#   --components overlays the component heatmap layer (e.g. for scenarios/examples/01_meadow.ron)

# Generational regime (P5) + dev generators (headless; the breeding one needs a `batch`):
cargo run --bin breed -- scenarios/deferred/breeding.ron [generations]    # run → score → breed; captures the best genome into species/saved/
cargo run --bin train                                                        # regenerate the deferred/learning showcase (+ the catalog variant)
cargo run --bin sweep -- scenarios/deferred/foodweb.ron                   # biodiversity sweep (seed / parameter) — search a coexistence band
