...
scenarios/        Two categories (Open ▸ Examples / Saved); only examples are committed.
  examples/       Curated, committed example scenarios:
    ...
    predator_prey.ron  3-level trophic chain (flora oases → prey → predators): count pyramid, shared Hunter brain, prey that flee (threat channel).
    ...
    # The MLP learning story, in three scenarios (mlp_brain & mlp_evolved are GENERATED by `cargo run --bin train`):
    mlp_brain.ron      Naive learned brain: a from-random MLP vs Wander on oasis flora → the wanderer out-forages it (the baseline before training).
    mlp_train.ron      Training ground: MLPs evolve ALONE on the oasis flora; the `train` bin captures an evolved individual to the catalog.
    mlp_evolved.ron    Trained variant in action: the captured MLP vs Wander → it reaches parity (no longer out-foraged). Cf. tests/mlp.rs.
